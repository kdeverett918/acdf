...
export default function Simulator() {
  ...
  }, [ran, age, levels, preDigest, prePRO]);

  const handleSimulate = () => setRan(true);
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="form-group">
            ...
            <input type="range" min={30} max={80} value={age} onChange={e => { setAge(+e.target.value); setRan(false); }} />
          </div>
          <div className="form-group">
            <label>Surgical Levels <span className="range-val">{levels}</span></label>
            <input type="range" min={1} max={4} value={levels} onChange={e => { setLevels(+e.target.value); setRan(false); }} />
          </div>
          <div className="form-group">
            <label>Pre-Op DIGEST <span className="range-val">{preDigest}</span></label>
            <input type="range" min={0} max={4} value={preDigest} onChange={e => { setPreDigest(+e.target.value); setRan(false); }} />
          </div>
          <div className="form-group">
            <label>Pre-Op PRO Severity <span className="range-val">{['None', 'Mild', 'Moderate', 'Severe'][prePRO]}</span></label>
            <input type="range" min={0} max={3} value={prePRO} onChange={e => { setPrePRO(+e.target.value); setRan(false); }} />
          </div>
        ...
        <div className="card" style={{ minHeight: 320 }}>
          {!results ? (
            ...
            <>
              <h3 style={{ marginBottom: '.75rem', fontSize: '.95rem' }}>Risk Estimate</h3>
              <div className="risk-meter" role="img" aria-label={`Overall risk: ${overallRisk.toFixed(0)}%`}>
              ...
              <div className="interp" style={{ marginTop: 0, padding: '1rem' }}>
