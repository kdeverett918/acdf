...
import { D, patients, measKeys, measLabels } from '@/lib/data';

function PatientExplorerInner() {
  ...
  const radarData = p ? (() => {
    ...
    measKeys.forEach(k => {
      const maxVal = k === 'swalqol' ? 100 : k === 'hssddi' ? 100 : k === 'eat10' ? 40 : k === 'mbsimp' ? 28 : k === 'bazaz' ? 3 : 4;
      if (k === 'swalqol') { pre.push((100 - (p[k + '_pre'] as number)) / 100 * 100); post.push((100 - (p[k + '_post'] as number)) / 100 * 100); }
      else { pre.push((p[k + '_pre'] as number) / maxVal * 100); post.push((p[k + '_post'] as number) / maxVal * 100); }
    });
    return { pre, post };
  })() : null;
          ...
          {!p ? (
            ...
            <>
              ...
              {Object.keys(D).map(k => {
                ...
                const maxVal = isSQ ? 100 : d.post.reduce((a, b) => Math.max(a, b), 0) || 1;
                return (
                ...
                {measKeys.map(m => {
                  ...
                  return (
                    <div key={m} style={{ padding: '.35rem', display: 'grid', placeItems: 'center' }}>
                      <div style={{ width: 26, height: 26, borderRadius: '50%', display: 'grid', placeItems: 'center', fontSize: '.65rem', fontWeight: 700, background: bg, color: col }} title={`${pt.label} ${m}: ${v}`}>{sym}</div>
                    </div>
                  );
                })}
              </React.Fragment>
...
export default function PatientExplorer() {
  return (
    <>
      ...
      <Suspense fallback={<div style={{ color: 'var(--text-dim)', textAlign: 'center', padding: '2rem' }}>Loading...</div>}>
        <PatientExplorerInner />
      </Suspense>
    </>
  );
}
